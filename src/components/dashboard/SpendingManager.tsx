...
import VoiceSpendingModal from "@/components/dashboard/VoiceSpendingModal";
import {
...
export default function SpendingManager({ periods, currency }: Props) {
  ...
  function openEdit(s: Spending) {
    setForm({
      ...
      spending_date: s.spending_date.toString().slice(0, 10),
      note: s.note ?? "",
    });
    setEditing(s);
    setError("");
          ...
          prev.map((s) =>
            s.id === editing.id
              ? {
                  ...
                  category_color: editing.category_color,
  ...
  const totalSpent = spendings.reduce(
    (sum, s) => sum + parseFloat(String(s.amount)),
    0,
  );
    ...
    <>
          ...
          budgets.length > 0 && (
            <div className="flex gap-2">
              ...
              <button
                ...
                className="button-secondary text-sm px-3 py-2 flex items-center gap-1.5"
                aria-label="Add spending with voice note in Darija"
              >
                <svg
      ...
      {!selectedPeriodId && (
        <div className="card py-16 text-center text-sm text-muted">
          Select a period to view spendings.
        </div>
      )}
        ...
        <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-6">
            ...
            <div className="card !py-3 hidden sm:block">
              ...
              <p className="text-xl font-extrabold text-primary">
                {new Set(spendings.map((s) => s.category_name)).size}
              </p>
            </div>
          </div>
          ...
          {spendings.length === 0 ? (
            <div className="card py-12 text-center text-sm text-muted">
              {budgets.length === 0
                ? "No budgets for this period. Create budgets first."
                : "No spendings yet. Add your first spending above!"}
            </div>
          ) : (
                    ...
                    {grouped[date].map((s, i) => (
                      ...
                      >
                        <span
                          ...
                          style={{
                            backgroundColor: `${s.category_color ?? "#6d5aee"}22`,
                            color: s.category_color ?? "hsl(var(--accent))",
                          }}
                          aria-hidden="true"
                        >
                          {s.category_icon || "●"}
      ...
      <Modal
        open={modal !== null}
        onClose={closeModal}
        title={modal === "edit" ? "Edit Spending" : "New Spending"}
      >
              ...
              >
                ...
                {budgets.map((b) => (
                  <option key={b.id} value={b.id}>
                    {b.category_name} (limit:{" "}
                    {fmtAmount(parseFloat(String(b.max_amount)), currency)})
                  </option>
                ))}
              </select>
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, spending_date: e.target.value })
                }
                required
              />
            </div>
